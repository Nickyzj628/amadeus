...
const findModel = (input: string) => {
  ...
  const byProvider = MODELS.find((m) => m.provider.toLowerCase() === normalized);
  ...
  return MODELS.find(
    (m) =>
      m.model.toLowerCase().includes(normalized) || m.provider.toLowerCase().includes(normalized),
  );
};

const modelList = MODELS.map((m, i) => `${i + 1}. ${m.provider} - ${m.model}`).join("\n");
