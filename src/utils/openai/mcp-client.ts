...
const createTransport = (config: McpServerConfig): Transport => {
  ...
  // Stdio 传输（默认）
  const args = config.args.map((arg) => (arg.startsWith("src/") ? join(cwd(), arg) : arg));
...
const initMcpClient = async (serverName: string) => {
  ...
  const config = (appConfig.mcpServers as Record<string, McpServerConfig>)[serverName];
  if (!config) {
...
/** 获取指定 MCP 工具 */
export const getMcpTool = async (serverName: string, toolName: string): Promise<McpTool> => {
